...
class op_graph:
    ...
    def insert_op(self, current_op_node):
        if self.find_if_exist(current_op_node) == None:
            ...
            #self.attrs_dictionary[current_op_node.id] = current_op_node.attrs
    ...
    def traverse_and_calculate_per_op(self, ir_params, x, fw=True, bw=False):
        ...
        while len(available_op_queue) > 0 :
            ...
            #print(temp_op.performance_data["fw_value"])
            for key in temp_op.next.keys():
                #print(self.check_op_ready(temp_op.next[key][1]))
...
def construct_op_graph(ir_module):
    ...
    '''
    for tmp_op in computation_graph.call_nodes:
        print('op_name:' + str(tmp_op.id))
        for key in tmp_op.prior.keys():
            print('op_pre:' + str(tmp_op.prior[key][1].id) + '&' + str(tmp_op.prior[key][1].type))
    '''
    '''
    for tmp_op in computation_graph.call_nodes:
        print('op_name:' + str(tmp_op.id))
        for key in tmp_op.prior.keys():
            print('op_pre:' + str(tmp_op.prior[key][1].id) + '&' + str(tmp_op.prior[key][1].type))
        for key in tmp_op.next.keys():
            print('op_next:' + str(tmp_op.next[key][1].id) + '&' + str(tmp_op.next[key][1].type))
...
def recursive_traverse_op(attrs, args, temp_op=None):
    ...
    '''
    if next_op_node.id == "multiply-114":
        print(attrs)
        print(args)
    '''
...
def profile_forward_relay_operator(ready_op_node, ir_params, x, dtype="float32"):
    ...
    #print(ready_op_node.op_instance.op)
    #help(temp_body)
    #print(temp_body.op)

...
> 当前工作：
>
>       * 研究TVM的具体框架结构，熟悉TVM编写规则。(之前TVM了解很浅，对研究工作造成了诸多障碍，反而影响进度，干脆花点时间，磨刀不误砍柴工)
>       * 学习之余，生成一些TVM的通用学习文档，弥补TVM手册不足的问题，便于后期组内成员跟进以及经验共享：https://github.com/dos-lab/TVMKnowledge
>       * 深入研究time_eveluate测算代码，涉及到Python与底层的C++代码接口调用。
>
> 下一步工作：
>
>       * 学习time_eveluate，看能否修改TVM源码，实现memory_eveluate测算
>       * 探究算子粒度异构计算的TVM调用规则
>
> 已经完成：
>
...
>       * 基于TVM对ONNX模型解析为计算图
>       * 更高精度的算子运行时间测算，基于TVM的time_eveluate，误差10%
>       * 针对常见算子的推理时间生成训练集（反向传播很复杂，TVM现有了解可能不足以支撑，暂未研究）
>       * 了解近年一些相关的深度学习优化工作：线程数量-运行效率、CPU-GPU异构推理等
>
> 当前遇到的问题：
>
>       * 针对运行时间的测算需要评估反向传播，针对TVM的了解，包括代码实现、具体能做到的事都十分模糊，已知的调用规则还不足以支撑完整的科研任务。需要花具体的时间，深入了解TVM框架结构，消除这个阻碍再进一步考虑科研任务。因此近期决定暂停研究工作，转向深入理解TVM。
